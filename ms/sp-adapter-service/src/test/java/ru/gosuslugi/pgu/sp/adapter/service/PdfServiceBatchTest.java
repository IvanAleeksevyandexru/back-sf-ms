...
import static ru.gosuslugi.pgu.sp.adapter.util.DefaultOptionsSpConfig.*;

// TODO KK Перенести тесты в модуль pdf-generator-ms во время удаления генерации PDF из SP-Adapter
@Slf4j
public abstract class PdfServiceBatchTest {
    ...
    public PdfServiceBatchTest(String roleId, Long oid, String directory, TestPdfMode testPdfMode) throws IOException {
        this.roleId = roleId;
        this.oid = oid;
    ...
    public void testPdf() throws IOException {
        ...
        pdfFileDescription.setType(FileType.COMMON_PDF);
        String pdfAttachmentFilename = PDF_ATTACHMENT_NAME;
        pdfFileDescription.setFileName(pdfAttachmentFilename);
        pdfFileDescription.setAddedFileName(UniqueType.NONE);
        pdfFileDescription.setMnemonic(PDF_ATTACHMENT_NAME);
        pdfFileDescription.setAddedMnemonic(UniqueType.NONE);
        pdfFileDescription.setAttachmentType(AttachmentType.LK);
        Map<ApplicantRole, String> templatesPdf = Map.of(
                ApplicantRole.valueOf(roleId), determineTemplateFileName(templatesDataContext.getServiceId(), templatesDataContext.getServiceId(), ApplicantRole.valueOf(roleId), PDF_ATTACHMENT_TEMPLATE_PREFIX)
        );
        pdfFileDescription.setTemplates(templatesPdf);

        // Generates PDF
        File pdfFile = pdfTemplateRenderService.createCommonPdfAttachment(templatesDataContext, pdfFileDescription);
        ...
        if (System.getProperty("os.name").toLowerCase().contains("linux")) {
            Runtime.getRuntime().exec(new String[]{"/usr/bin/evince", pdfFile.getAbsolutePath()});
        }
