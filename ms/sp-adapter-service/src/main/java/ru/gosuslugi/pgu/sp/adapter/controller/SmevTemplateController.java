...
public class SmevTemplateController {
     ...
     * <p>
     * Service processing adapter retrieves draft from draft service, processes service template and
     * sends it to epgu service processing module.
     */
    ...
    public ResponseEntity<?> sendDraftToSmev(
            @RequestBody
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "DTO SpAdapterDto",
                    content = @Content(schema = @Schema(implementation = SpAdapterDto.class)))
            SpAdapterDto dto) {
    ...
    public ResponseEntity<?> sendDraftBatchToSmev(
            @Parameter(in = ParameterIn.PATH, description = "ID of service")
            @PathVariable String serviceId,

            @RequestBody
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "List of DTO SpAdapterDto",
                    content = @Content(array =
                    @ArraySchema(schema = @Schema(implementation =SpAdapterDto.class))))
    ...
    public ResponseEntity<?> sendSignedSmevRequest(
            @RequestBody
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "DTO SpAdapterDto",
                    content = @Content(schema = @Schema(implementation = SpAdapterDto.class)))
            SpAdapterDto dto) {
        return smevTemplateEngineService.processSignedSmevRequest(dto.getServiceId(),
                dto.getOrderId(), dto.getOid(), dto.getRole(), dto.getOrgId(), false)
                ? ResponseEntity.ok().build()
                : ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
    }
    ...
    public SmevRequestDto createXmlAndPdf(
            @RequestBody
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "DTO SpAdapterDto",
                    content = @Content(schema = @Schema(implementation = SpAdapterDto.class)))
            SpAdapterDto dto) {
        return smevTemplateEngineService.createXmlAndPdf(dto.getOrderId(), dto.getOid(),
                dto.getOrgId(), dto.getRequestGuid(), false);
    }
}
